...
public class PageElement {
	 ...
	 * @param selectors   Map the various selectors to iterate through to find the
	 *                    element
	 */
			...
			if (!"elementType".equalsIgnoreCase(locatorType)) {
				try {
					...
					var errorMessage = SentinelStringUtils.format(
							"{} is not a valid selector type. Please fix the element {} in the {}.yml page object.",
							locatorType, elementName, PageManager.getPage().getName());
	...
	protected WebElement element() {
		...
		while ((System.currentTimeMillis() - startTime) < Time.out() * 1000) {
			for (Map.Entry<SelectorType, String> selector : selectors.entrySet()) {
				log.trace("Attempting to find {} {} with {}: {}", elementType, getName(), selector.getKey(),
						selector.getValue());
				element = getElementWithWait(createByLocator(selector.getKey(), selector.getValue()),
	...
	/**
	 * This method is used to get the tool tip text on mouse hover
	 * 
	 ...
	 * 
	 * @param text String (keys to type)
	 * @return PageElement (for chaining)
	...
	/**
	 * TODO: Test to make sure this to work with multiple selectors Determines with
	 * 250 milliseconds (1/4 of a second) if an element is not present. This should
	 * be used when you expect an element to not be present and do not want to slow
	 * down your tests waiting for the normal timeout time to expire.
	 * 
	...
	public boolean attributeEquals(String attribute, String value) {
		...
		log.debug("Values found for attribute {} on element {}: {}", attribute, this.getClass().getName(), values);
