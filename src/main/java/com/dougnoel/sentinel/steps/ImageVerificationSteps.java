...
public class ImageVerificationSteps {
    ...
    private static boolean checkIfImagesMatch(String elementName, boolean negate, String optionalStoredImage) {
		...
		if(optionalStoredImage != null){
			testDataImageLocation = FileManager.findFilePath(Configuration.getTestData("images", optionalStoredImage));
		}
		...
		boolean debugBoolean = Configuration.toBoolean("debug");
		if(debugBoolean) {
			try {
				FileManager.saveImage(outputFolder, imageId + "_" + "EXPECTED" + ".png", comparisonImage);
				FileManager.saveImage(outputFolder, imageId + "_" + "ACTUAL" + ".png", currentStateImage);
			}
			catch(Exception e) {
