 ...
 * 
 * Functionality in this class includes clicking given elements, entering text,
 * selecting items, verifying elements exist, contains given text, or are
 * active, enabled, or hidden, verifying table columns or table rows have given
 * text, unique text, text for stored values, navigating results, and basic
 * browser interaction.
 */
public class BaseSteps {
	...
	/**
	 * Clicks the element that matches the given elementName as defined on the
	 * current Page object. The page object and driver object are defined by the
	 ...
	 * PageElement object (e.g. login_button).
	 * <p>
	...
	public void mouseoveronTarget(String elementName) {
		...
		getElement(elementName).getToolTipText();
	}
	...
	public static void wait(double seconds) {
		...
		Time.wait(seconds);
		log.warn(
				"Passed {} seconds, waiting {} milliseconds. Waits should only be used for special circumstances. If you are seeing this message a lot then you should probably be logging a bug ticket to get the framework fixed at: http://https://github.com/dougnoel/sentinel/issues",
				seconds, (seconds * 1000));
	}

	/**
	 * Loads a page based on the environment you are currently testing. The url is
	 * set in the page object yaml file. Refer to the documentation in the
	 * sentinel.example project for more information. You cannot load a URL
	 ...
	 * <li>I am on the Home Page using the arguments
	 * ?firstname=bob{@literal &}lastname=smith</li>
	 * </ul>
	 * 
	 * @param pageName     String Page Object Name
	 * @param hasArguments boolean indicates whether there is a query string to add
	 *                     to the usual URL
	...
	/**
	 * Overloaded method for navigating to a page object's url based on the current
	 * environment, so that nulls do not need to be passed. Intended for use in
	 * creating complex Cucumber steps definitions.
	 * 
	 * @param pageName String Page Object Name
	 */
	...
	/**
	 * Overloaded method for navigating to a page object's url based on the current
	 * environment, and taking a query string argument it will append to the
	 * request. Used so that nulls do not need to be passed. Intended for use in
	 * creating complex Cucumber steps definitions.
	 * 
	 * @param pageName  String Page Object Name
	 * @param arguments String the literal string to append to the default URL.
