...
public class TextVerificationSteps {
	...
	public static void verifyElementTextIsEmpty(String elementName, String assertion) {
		...
		var expectedResult = SentinelStringUtils.format("Expected the element {} to {}be empty.", elementName,
				(negate ? "not " : ""));
		if (negate) {
	 ...
	 * <li>I verify the tooltip contains the text "username"</li>
	 ...
	 * </ul>
	 * 
	 * @param elementName String The name of the element to be evaluated as defined
	 *                    in the page object.
	 * @param assertion   String Evaluated as a boolean, where null = false and any
	 *                    text = true.
	 * @param tooltip     String Evaluated as a boolean, where null = false and any
	 *                    text = true.
	 * @param matchType   String whether we are doing an exact match or a partial
	 *                    match
	 * @param text        String The text to verify exists in the element.
	 */
	@Then("^I verify the (.*?)( tooltip)?( does not)? (has|have|contains?) the text \"([^\"]*)\"$")
	public static void verifyElementTextContains(String elementName, String tooltip, String assertion, String matchType,
			String text) {
		boolean negate = !StringUtils.isEmpty(assertion);
		...
		if (elementName.contains("URL")) {
			...
			if (hasTooltip) {
				...
				var expectedResult = SentinelStringUtils.format(
						"Expected the {} element to {}{} the text {}. The tooltip element contained the text: {}",
						elementName, negateText, partialMatchText, text, elementText.replace("\n", " "));
	...
	private static void verifyURLTextContains(String text) {
		...
		var expectedResult = SentinelStringUtils.format("Expected the URL {} to contain the text \"{}\".", currentUrl,
				text);
		log.trace(expectedResult);
	...
	/**
	 * Used to verify that the selected option of a select element contains the text
	 * used in an element. It takes an element name and then an optional "does not",
	 * which if present means that the method will look for the text to not exist.
	 * Then it will look up the value set for the second element. This second
	 * element may be the same element or a different one. For example, you might
	 * select the first item in a drop down and not know what you selected. Using
	 * this method, you can check to make sure that the value appears correctly.
	 * Alternately, you might enter a value in a text box and want to verify that it
	 * was programmatically selected in a drop down.
	 * <p>
	 * <b>Gherkin Examples:</b>
	 * <ul>
	 * <li>I verify the City Dropdown has the value selected for the City
	 * Dropdown</li>
	 * <li>I verify the City Dropdown has the value entered for the City Field</li>
	 * <li>I verify the Shipping City Dropdown does not have the value used for the
	 * Billing City Dropdown</li>
	 * </ul>
	 * 
	 * @param elementName String Name of the Element to verify
	 * @param assertion   String if empty we expect this to be false
	 * @param key         String the key to retrieve the text to match from the
